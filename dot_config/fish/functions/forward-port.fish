...
#
# Usage:
#   forward-port [--verbose] <gateway> <remote-host> <remote-port> [local-port]
#
# Arguments:
...
#
# Examples:
#   forward-port headnode gpu1 8888
#     → Forwards localhost:8888 to gpu1:8888 via headnode
#
#   forward-port -v login-node compute5 6006 16006
...
#     alias jupyter='forward-port headnode gpu-node 8888'

# ------------------------------ main ------------------------------ 
function forward-port -d 'Forward a local port to a remote host via SSH gateway using autossh'
    set -l verbose 0
    set -l args
    for arg in $argv
        switch $arg
    ...
    if test -z "$remote" -o -z "$host" -o -z "$rport"
        echo "Usage: forward-port [--verbose] <gateway> <remote-host> <remote-port> [local-port]"
        return 1
    end
...
complete -c forward-port -d 'Set up local port forwarding to a host inside a remote via SSH gateway'
complete -c forward-port -f # disable file completion

# Argument 1: remote SSH gateway (e.g., headnode)
complete -c forward-port \
